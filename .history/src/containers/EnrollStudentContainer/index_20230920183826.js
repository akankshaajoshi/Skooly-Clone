...
function FilterableTable() {
  ...
  const handleDeleteClick = () => {
    const updatedStudents = fakeStudents.filter((student) => !selectedRows.includes(student.studentId));
    // Call the deleteStudentData function here with the selected student IDs
    const selectedStudentIds = selectedRows.map((studentName) => {
      const selectedStudent = fakeStudents.find((student) => student.name === studentName);
      return selectedStudent ? selectedStudent.studentId : null;
    });
