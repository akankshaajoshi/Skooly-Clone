...
function Index() {
  ...
  useEffect(() => {
    if (firstRender) {
      console.log('First render:', fakeStaffs);
      setFirstRender(false); // Set to false after the first render
    }
  }, [firstRender, fakeStaffs]);

  // Conditionally render data based on firstRender
  const data = useMemo(
    () => (firstRender ? [[...fakeStaffs]] : [...fakeStaffs]),
    [firstRender, fakeStaffs],
  );
