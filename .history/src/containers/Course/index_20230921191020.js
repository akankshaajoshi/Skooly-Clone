...
function FilterableTable() {
  const { data: fakeCourses, isLoading, isError, error } = useQuery('courseData', fetchCourseData);
  ...
  return (
    <FilterableTableContainer>
      ...
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: '10px' }}>
        <Input type="text" placeholder="Filter by Course Name" name="courseName" value={filter.courseName} onChange={handleFilterChange} />
          ...
          <Table>
            ...
            <tbody>
              {filteredCourses.map((course, index) => (
                <tr key={index}>
                  <Td>
                    <input type="checkbox" checked={selectedRows.includes(course.courseName)} onChange={() => handleCheckboxChange(course.courseName)} />
                  </Td>
