...
const index = () => (
  <Container style={containerStyle}>
    <Heading style={{ fontSize: fontSizes.xLarge }}>
      Download Skooly mobile app
    </Heading>
    <CardContainer>
      <Card
        style={{ backgroundColor: `${colors.primary}`, paddingLeft: '30px' }}
      >
        <CardText>
          <Heading style={{ fontSize: fontSizes.medium }}>
            App for learners, students or parents
          </Heading>
          <ButtonContainer>
            {downloads.map((download, ind) => (
              <StyledLink key={ind} to={download[0]}>
                <Button>
                  {' '}
                  <Img src={ind === 1 ? img : img2} alt="download" />
                  {download[1]}
                </Button>
              </StyledLink>
            ))}
      ...
      <Card
        style={{ backgroundColor: `${colors.secondary}`, paddingRight: '30px' }}
      >
        <CardImage src={imgb} />
        <CardText style={{ color: `${colors.white}` }}>
          <Heading style={{ fontSize: fontSizes.medium }}>
            App for schools, teachers, coaches
          </Heading>
          <ButtonContainer>
            {downloads.map((download, ind) => (
              <StyledLink key={ind} to={download[0]}>
