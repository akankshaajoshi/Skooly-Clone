...
const generatePersonsData = (number) => {
  ...
  function generateStudentData(count) {
    ...
    for (let i = 0; i < count; i++) {
      const studentId = "TESSE"+faker.string.number()
      const student = {
        id: i,
        ...
        studentId: studentId, // Random category
        dob: faker.date.past(),// Random pricing option
        mother: { 
          email: faker.internet.email(),
          number: faker.datatype.number({min: 1000000000, max: 9999999999})
        },
        father: {
            email: faker.internet.email(),
            number: faker.datatype.number({min: 1000000000, max: 9999999999})
